...
- Корневой `Site` div: `max-w-[1240px] mx-auto px-5` — ограничивает, центрирует и добавляет отступы
- Body background: `bg-surface` (`#0a0a0a`) — обеспечивает единый тёмный фон на широких экранах
- Полноширинные фоны (Header, HeroBlock, Footer): абсолютный div `w-screen left-1/2 -translate-x-1/2`
- `overflow-x-clip` на `motion.div` в InteractiveSite — предотвращает горизонтальный скролл от `w-screen`
